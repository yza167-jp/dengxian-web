...
真实验证：2026-07-31 已用仅服务端读取的 DeepSeek Key 完成一次最小 JSON 动作探针和 Safari 对局，均返回合法动作且未触发本地回退；模型、延迟与最低成本调用记录见 `docs/TEST_REPORT.md`。部署到新环境时仍应使用设置页固定探针复核该环境的 Key、模型可用性与网络出口。
